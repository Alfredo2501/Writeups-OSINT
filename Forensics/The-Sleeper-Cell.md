...
```
Find the meaning of the message.
Sample answer for the flag: the flag will be the text in middle of parenthesis, example: ((this-is-the-flag)) [Including the parenthesis]
```

### Decoding the message

The message is in Base64 code, so, we can use a Base64 decoder to do the job, we can use a decoder online, I'll use _https://www.base64decode.org_:

![Base64](https://github.com/user-attachments/assets/7a2990e7-f1f9-4c27-9691-2710343eddca)

From the way the message is written, we can deduce that the message has Vigenere encryption, if you don't know the Vigenere encryption, you can read this article about the encryption: _https://en.wikipedia.org/wiki/Vigen%C3%A8re_cipher_

Again, we have to decode the message, now in Vigenere encryption, same as with Base64 encryption, we have to use a decoder, I'll use _https://www.dcode.fr/vigenere-cipher_


Unlike Base64, we need a key to decode the Vigener encryption, and what is the key?. After a while searching in the information about the case and the material, I deduced the key could be the name of the group: "**Androktasiai**". The way the group name is written looks very similar with the key of Vigenere encryption, so I used the name as the key and I get the result:

![Vigenere](https://github.com/user-attachments/assets/d658a191-d9f7-4a3c-bccb-deba93f2adb0)

And now we have the next message:
...
As we can see, the flag is a coordinates about a stadium called SoFi Stadium, localed in USA, California.
